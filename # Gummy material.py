...
class esprite():
    ...
    def particle_attraction(self):
        for other_particle in objlist:
            if other_particle!=self:
                maxdisqred=1000
                mg=5
                
                distsqred = (other_particle.rect.x)**2+(other_particle.rect.y)**2
##                xdir = abs(self.rect.x - other_particle.rect.x)/self.rect.x - other_particle.rect.x
##                ydir = abs(self.rect.y - other_particle.rect.y)/self.rect.y - other_particle.rect.y

                if other_particle:
                    if self.rect.x - other_particle.rect.x>0:
                        xdir=-1
                    else:
                        xdir=1
                        
                    if self.rect.y - other_particle.rect.y>0:
                        ydir=-1
                    else:
                        ydir=1                

                    if distsqred!=0:  
                        self.vel[0] += (xdir/(distsqred))*mg - xdir*(((distsqred))**0.500000002)
                        self.vel[1] += (ydir/(distsqred))*mg - ydir*(((distsqred))**0.500000002)
